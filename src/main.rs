    ...
    Command::new("latency")
        ...
        .subcommand(
            Command::new("server")
                ...
                .arg(arg!(-p --port <PORT> "server port").value_parser(value_parser!(usize)).default_value("65432"))
        ...
        .subcommand(
            Command::new("client")
                ...
                .arg(arg!(-p --port <PORT> "server port").value_parser(value_parser!(usize)).default_value("65432"))
                .arg(arg!(-u --udp "if this arg is set, use udp, else tcp"))
                .arg(arg!(-c --count <COUNT> "test how many times").value_parser(value_parser!(usize)).default_value("100"))
...
fn main() {
    ...
    match matches.subcommand() {
        ...
        Some(("client", sub_matches)) => {
            let port = *sub_matches.get_one::<usize>("port").unwrap();
            let server_ip = sub_matches.get_one::<String>("SERVER_IP").expect("required");
            let is_udp = *sub_matches.get_one::<bool>("udp").unwrap();
